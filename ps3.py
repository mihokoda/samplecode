...
def get_word_score(word, n):
    ...
    first_comp = 0
    for i in word:
        first_comp= first_comp + SCRABBLE_LETTER_VALUES.get(i)
...
def display_hand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            str = str + letter + ' ' 
    return str                              # print an empty line
    ...
    for i in wordc:
        if i not in handc.keys():
            ...
            if handc[i]!=1:
                ...
                del handc[i]
    if "*" in wordc:
        if wordc.replace("*", "a") not in word_list and wordc.replace("*", "e") not in word_list and wordc.replace("*", "i") not in word_list and wordc.replace("*", "o") not in word_list and wordc.replace("*", "u") not in word_list:
            flag = False
    else:
        if wordc not in word_list: 
            flag = False
        ...
        if (len(current_hand)==0):
            ...
            if word == '!!':
                ...
                if is_valid_word(word, hand, word_list):
                    temp = get_word_score(word, len(hand.keys()))
                    total_points += temp
                    print(word, "earned", temp, "points. Total: ", total_points, "points.")
                else:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range (num_hand):
        ...
        if replay == 'yes':
            total_points+=play_hand(hand, word_list)
        else:
            total_points+=temp
